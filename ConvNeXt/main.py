...
def get_args_parser():
    ...
    parser.add_argument('--data_path', default='path/to/imagenet-1k', type=str,
                        help='dataset path')
    parser.add_argument('--eval_data_path', default='path/to/imagenet-1k', type=str,
    ...
    parser.add_argument('--focal', default=True)
...
def main(args):
    ...
    if args.inference:
        ...
        file_num = sorted(os.listdir(args.data_path), key=lambda x: int(x.split('-')[0]))

        for num in file_num:
            # 이미지 폴더 경로
            image_folder_path = f'{args.data_path}/{num}'
            csv_folder_path = f'{args.csv_path}/data_csv/{num}.csv'
            ...
            def load_images(image_folder):
                ...
                for filename in os.listdir(image_folder):
                    if filename.endswith('.jpg') or filename.endswith('.png'):
                        ...
                        image = preprocess(image)  # 이미지 전처리 (리사이즈 및 텐서로 변환)
            ...
            # 이미지 텐서들을 하나의 배치로 묶기 (N, C, H, W 형식)
            try:
            ...
            # CSV 파일 열기
            with open(csv_folder_path, mode='r', encoding='utf-8') as csv_file:
                csv_reader = csv.reader(csv_file)
                
                # 첫 번째 행은 열 이름이므로 건너뛰기
                next(csv_reader)
            ...
            # 두 리스트를 합치는 새로운 리스트 생성
            merged_list = []

            for index in range(len(preds)):
                merged_list.append([str(preds[index])] + data_list[index])
            # 결과 출력
            # print(merged_list)
            # csv 파일 만들고 열이름, 데이터 넣기
            results = args.results_dir
            os.makedirs(results, exist_ok=True)
            with open(f'{results}/{num}.csv', mode='w', newline='') as file:
    ...
    if args.focal:
        ...
        weights = torch.FloatTensor([args.alpha1, args.alpha2])
        criterion = FocalLoss(gamma=args.gamma, weights=weights, device=device)
    else:
    ...
    if args.eval:
        ...
        test_stats = evaluate(data_loader_val, model, device, args.nb_classes, use_amp=args.use_amp)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            if args.model_ema and args.model_ema_eval:
                test_stats_ema = evaluate(data_loader_val, model_ema.ema, device, args.nb_classes, use_amp=args.use_amp)
