...
@torch.no_grad()
def evaluate(data_loader, model, device, nb_classes, use_amp=False):
    ...
    #     p = preds.cpu().item()
    #     img_path, t = data_loader.dataset.imgs[idx]
    #     if t != p:
    #         # img_path, _ = data_loader.dataset.imgs[idx]
    #         img_name = img_path.split('/')[-1][:-4]
    #         wrong1.append(img_name)
    ...
    for i in range(num_classes):
        print(f'Class {i} accuracy: {class_accuracy[i]:.3f}')
